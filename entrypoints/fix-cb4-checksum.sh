...
while read byte; do
	...
	pos=$(( $pos + 1 ))
done <<< $(dd if="$mcs_file" bs=1 count=6784 skip=640 | od -v -An -tu1 -w1)

checksum=$(printf "%08X" $checksum)

echo "CB4 checksum: ${checksum}"

echo -ne "\x${checksum:6:2}\x${checksum:4:2}\x${checksum:2:2}\x${checksum:0:2}" | dd conv=notrunc of="$mcs_file" bs=1 seek=6784
